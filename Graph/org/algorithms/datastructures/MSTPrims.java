...
public class MSTPrims {
	...
	void primMST(int graph[][]) {
		...
		keyWeight[0] = 0; // Make key 0 so that this vertex is
		...
		for (int count = 0; count < V - 1; count++) {
			...
			for (int v = 0; v < V; v++)
				...
				if (graph[u][v] != 0 && mstSet[v] == false && graph[u][v] < keyWeight[v]) {
					parent[v] = u;
					keyWeight[v] = graph[u][v];
				}
		}
